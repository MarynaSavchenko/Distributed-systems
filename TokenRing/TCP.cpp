...
int main(int argc, char **argv) {
    ...
    while(true) {
        if (epoll_wait(epoll_fd, &ev, 1, -1) == -1)//1 event , timeout:block indefinitely
...
void create_server(){
    ...
    sockfd_in = socket(AF_INET, SOCK_STREAM, 0);//IPv4 protocol, stream, protokol
    if (sockfd_in < 0)
    ...
    listen(sockfd_in,5);//max length queue
}
